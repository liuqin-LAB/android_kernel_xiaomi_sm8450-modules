...
static bool trace_wake;
module_param(trace_wake, bool, 0644);
MODULE_PARM_DESC(trace_wake, "Log resume timing and the first packet/touch after wake");
...
static void nvt_irq_wake_enable(void)
{
	int ret;

	mutex_lock(&ts->irq_lock);
	if (!ts->irq_wake_enabled && device_may_wakeup(&ts->client->dev)) {
...
int32_t nvt_check_fw_reset_state(RST_COMPLETE_STATE check_reset_state)
{
	...
	if (READ_ONCE(trace_wake))
		NVT_LOG("wake reset target=0x%02x state=0x%02x retries=%d elapsed_ms=%lld ret=%d\n",
			check_reset_state, buf[1], retry,
			ktime_ms_delta(ktime_get(), start), ret);
...
void nvt_ts_wakeup_gesture_report(uint8_t gesture_id, uint8_t *data)
{
	...
	switch (gesture_id) {
		...
		case GESTURE_DOUBLE_CLICK:
			...
			if (READ_ONCE(ts->doubletap_enabled) &&
			    (READ_ONCE(ts->gesture_command) & BIT(0)))
...
void nvt_ts_pen_gesture_report(uint8_t pen_gesture_id)
{
	...
	if (pen_gesture_id == PEN_GESTURE_ONE_CLICK &&
	    READ_ONCE(ts->pen_shorthand_enabled) &&
	    READ_ONCE(ts->pen_bluetooth_connect) &&
	    (READ_ONCE(ts->gesture_command) & BIT(1))) {
...
static int nvt_send_host_mode_locked(u8 command, u8 value)
{
	u8 buf[3] = { EVENT_MAP_HOST_CMD, command, value };
	int ret;

	if (READ_ONCE(trace_wake))
		NVT_LOG("wake host command=0x%02x value=%u\n", command, value);

	ret = nvt_set_page(ts->mmap->EVENT_BUF_ADDR | EVENT_MAP_HOST_CMD);
	if (ret < 0)
		return ret;

	return CTP_SPI_WRITE(ts->client, buf, sizeof(buf));
}

static int nvt_apply_pen_status_locked(bool force)
{
	bool enable;
	int ret = 0;

	lockdep_assert_held(&ts->lock);
	mutex_lock(&ts->pen_switch_lock);
	if (!bTouchIsAwake || READ_ONCE(ts->teardown_in_progress) ||
	    smp_load_acquire(&ts->dev_pm_suspend))
		goto out;

	enable = ts->pen_bluetooth_connect && !ts->pen_charge_connect &&
		 !ts->game_mode_enable;
	if (!force && ts->pen_enable_last_status == enable)
		goto out;
...
static int update_pen_status(bool force)
{
	int ret;
	...
	mutex_lock(&ts->lock);
	ret = nvt_apply_pen_status_locked(force);
	mutex_unlock(&ts->lock);
	return ret;
}
...
static void nvt_esd_check_func(struct work_struct *work)
{
	...
	if ((timer > NVT_TOUCH_ESD_CHECK_PERIOD) && esd_check) {
		...
		nvt_apply_pen_status_locked(true);
		mutex_unlock(&ts->lock);
		/* update interrupt timer */
		irq_timer = jiffies;
...
static irqreturn_t nvt_ts_work_func(int irq, void *data)
{
	...
	if (ts->wake_first_packet && bTouchIsAwake) {
		ts->wake_first_packet = false;
		NVT_LOG("wake first packet ready_ms=%lld type=0x%02x\n",
			ktime_ms_delta(ktime_get(), ts->wake_ready_time),
			point_data[1]);
	}
/*
	//--- dump SPI buf ---
...
*/
   ...
   if (POINT_DATA_LEN >= POINT_DATA_CHECKSUM_LEN) {
	...
	if (ts->wake_first_touch && finger_cnt) {
		ts->wake_first_touch = false;
		NVT_LOG("wake first touch ready_ms=%lld fingers=%d\n",
			ktime_ms_delta(ktime_get(), ts->wake_ready_time),
			finger_cnt);
	}
...
static int nvt_set_mode_value(void *private, enum touch_mode mode, int value)
{
	...
	case TOUCH_MODE_REPORT_RATE:
		...
		if (mode == TOUCH_MODE_REPORT_RATE) {
			nvt->game_mode_requested = value;
			queue_work(nvt->set_touchfeature_wq, &nvt->set_touchfeature_work);
			goto out;
		}
		if (mode == TOUCH_MODE_DOUBLETAP_GESTURE)
			nvt->doubletap_enabled = value;
		else
			nvt->pen_shorthand_enabled = value;
		break;
	case TOUCH_MODE_STYLUS_CONNECTION:
		if (value >= 0x13 && value <= 0x17)
			value = 0x12;
		switch (value) {
		case -1:
			nvt->pen_count = 0;
			break;
		case 0x01:
			nvt->pen_shield_flag = false;
			break;
		case 0x11:
			nvt->pen_shield_flag = true;
			break;
		case 0x02:
		case 0x12:
			if ((value == 0x12 && nvt->pen_count == INT_MAX) ||
			    (value == 0x02 && nvt->pen_count == INT_MIN)) {
				ret = -EOVERFLOW;
				goto out;
			}
			nvt->pen_count += value == 0x12 ? 1 : -1;
			break;
		default:
			break;
		}
		nvt->pen_bluetooth_connect = nvt->pen_count != 0 &&
					     !nvt->pen_shield_flag;
		break;
	default:
		ret = -EINVAL;
	...
	gesture_mask = nvt->doubletap_enabled ? BIT(0) : 0;
	if (nvt->pen_shorthand_enabled && nvt->pen_bluetooth_connect)
		gesture_mask |= BIT(1);
	nvt_set_gesture_mode_locked(nvt, gesture_mask);
	if (mode == TOUCH_MODE_STYLUS_CONNECTION) {
		/* The delta is consumed even if the controller transfer fails. */
		nvt_apply_pen_status_locked(false);
		release_pen_event();
	}
out:
...
static int nvt_get_mode_value(void *private, enum touch_mode mode)
{
	...
	switch (mode) {
	case TOUCH_MODE_DOUBLETAP_GESTURE:
		value = nvt->doubletap_enabled;
		break;
	case TOUCH_MODE_REPORT_RATE:
		value = nvt->game_mode_requested;
		break;
	case TOUCH_MODE_STYLUS_CONNECTION:
		value = nvt->pen_bluetooth_connect;
		break;
	case TOUCH_MODE_PEN_SHORTHAND:
...
static void update_touchfeature_value_work(struct work_struct *work)
{
	struct nvt_ts_data *nvt = container_of(work, struct nvt_ts_data,
					     set_touchfeature_work);
	int ret;

	mutex_lock(&nvt->lock);
	if (!bTouchIsAwake || READ_ONCE(nvt->teardown_in_progress) ||
	    smp_load_acquire(&nvt->dev_pm_suspend) ||
	    (!nvt->game_mode_dirty &&
	     nvt->game_mode_requested == nvt->game_mode_enable)) {
		mutex_unlock(&nvt->lock);
		return;
	}
	ret = nvt_send_host_mode_locked(0x7A, nvt->game_mode_requested);
	if (!ret) {
		nvt->game_mode_enable = nvt->game_mode_requested;
		nvt->game_mode_dirty = false;
	}
	mutex_unlock(&nvt->lock);
	msleep(35);
	msleep(40);
	if (ret < 0) {
		NVT_ERR("report rate change failed: %d\n", ret);
		return;
	}
		...
		unsigned long event, void *data)
{
	...
	WRITE_ONCE(nvt->pen_charge_connect, !!event);
...
{
	struct nvt_ts_data *nvt =
		container_of(work, struct nvt_ts_data, power_supply_work);
	int supplied;
	int ret;
	u8 buf[3] = { EVENT_MAP_HOST_CMD, 0, 0 };

	if (READ_ONCE(nvt->teardown_in_progress))
		return;

	mutex_lock(&nvt->power_supply_lock);
	if (!READ_ONCE(bTouchIsAwake) || READ_ONCE(nvt->teardown_in_progress) ||
	    smp_load_acquire(&nvt->dev_pm_suspend))
		goto out;
	...
	if (bTouchIsAwake && !READ_ONCE(nvt->teardown_in_progress) &&
	    !smp_load_acquire(&nvt->dev_pm_suspend) &&
	    supplied != nvt->is_usb_exist) {
		buf[1] = supplied ? 0x53 : 0x51;
		ret = CTP_SPI_WRITE(nvt->client, buf, sizeof(buf));
		if (ret < 0)
			NVT_ERR("USB status set failed: %d\n", ret);
		else
			nvt->is_usb_exist = supplied;
	}
	mutex_unlock(&nvt->lock);
out:
	mutex_unlock(&nvt->power_supply_lock);
}
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	ts->set_touchfeature_wq = alloc_workqueue("nvt-touchfeature",
	...
	ts->xiaomi_touch.gesture_mode_before_suspend = true;
	ret = register_xiaomi_touch_client(TOUCH_ID_PRIMARY, &ts->xiaomi_touch);
	if (ret)
		goto err_register_touchfeature_failed;
...
err_register_touchfeature_failed:
...
static int32_t nvt_ts_resume(struct device *dev)
{
	...
	NVT_LOG("start\n");
	tracing = READ_ONCE(trace_wake);
	restore_start = ktime_get();
	...
	bTouchIsAwake = 1;
	ready = ktime_get();
	ts->wake_ready_time = ready;
	ts->wake_first_packet = tracing;
	ts->wake_first_touch = tracing;
	nvt_irq_wake_disable();
	mutex_unlock(&ts->lock);
	if (tracing)
		NVT_LOG("wake ready wait_ms=%lld restore_ms=%lld total_ms=%lld gesture=0x%x\n",
			ktime_ms_delta(restore_start, start),
			ktime_ms_delta(ready, restore_start),
			ktime_ms_delta(ready, start), gesture_command);
	...
	if (ts->game_mode_requested || ts->game_mode_enable)
		ts->game_mode_dirty = true;
	queue_work(ts->set_touchfeature_wq, &ts->set_touchfeature_work);
	mutex_unlock(&ts->lock);
	mutex_unlock(&ts->transition_lock);
